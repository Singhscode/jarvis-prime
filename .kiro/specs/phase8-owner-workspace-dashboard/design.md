...
`POST /api/owner-workspace/clients` remains the single Owner collection endpoint. Bodies containing `lead_id` continue to delegate unchanged to `crm.createClient(ownerUserId, { lead_id, name })`, which retains the existing conversion RPC. Bodies without `lead_id` are strictly validated by a new CRM direct-create service method and insert the same `crm_clients` entity with server-derived `owner_user_id`. The legacy `/api/crm/clients` endpoint remains conversion-only.

The additive schema stores direct-client email, phone, company, and optional notes on `crm_clients`, plus one sequence-backed, unique display `client_code` generated by PostgreSQL. UUID `id` remains the primary key and authorization locator. Direct-only fields remain nullable for compatibility with existing conversion inserts. The Clients page owns a local accessible creation dialog and refreshes its in-memory list after success; it adds no profile or related-resource behavior.