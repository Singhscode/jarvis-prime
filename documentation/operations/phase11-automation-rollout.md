...
## Production migration authorization: 20260810000023 through 20260810000031, then 20260810000035, then 20260810000036, then 20260810000038, then 20260810000039, then 20260810000040
Historical migrations `20260810000023`–`20260810000031` are immutable. The only approved forward production sequence is `20260810000023` → `…00031` → `20260810000035_complete_phase11_local_candidate_controls.sql` → `20260810000036_harden_phase11_p0_controls.sql` → `20260810000038_add_automation_resource_trigger_contract.sql` → `20260810000039_complete_core_automation_engine.sql` → `20260810000040_harden_automation_worker_claim_drain.sql`. Migration 35 supplies the idempotent eight-argument control RPC, immutable audit evidence, and controlled `INTERNAL_FAKE` future-trigger path; migration 36 supplies globally keyed control idempotency receipts, UTC-day DAILY reservation rebinding, and request-bound future-trigger replay protection; migration 38 enforces the automation resource trigger contract and runtime validation; migration 39 completes the core automation engine with durable run state transitions; migration 40 hardens worker claim draining and lease recovery safety.

`20260810000037_add_phase11_internal_fake_canary.sql` is retained only as historical source outside the static rollout manifest and unified CI ledger assertions. It is not executable through the production gate or any repository canary path. The production gate continues to stop if 37 appears in the production ledger; its approved source subset remains exactly `31 → 35 → 36 → 38 → 39 → 40`.
...
- `20260810000038_add_automation_resource_trigger_contract.sql` — `9efb3ba4b524ed67f45ded904aa8355687adbef7ee6a131829a456931e16fa24`
...
8. **Apply 38 only:** after successful 36 verification, apply exactly 38 using the same transaction-and-ledger-record behavior.
9. **Verify 38:** the runner reopens a read-only ledger transaction and requires the verified 38 ledger record.
10. **Apply 39 only:** after successful 38 verification, apply exactly 39 using the same transaction-and-ledger-record behavior.
11. **Verify 39:** the runner reopens a read-only ledger transaction and requires the verified 39 ledger record.
12. **Apply 40 only:** after successful 39 verification, apply exactly 40 using the same transaction-and-ledger-record behavior.
13. **Verify 40:** the runner reopens a read-only ledger transaction and requires the verified 40 ledger record.
