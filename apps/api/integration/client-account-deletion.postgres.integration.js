...
async function fixture() {
  ...
  await db.query(`insert into public.crm_leads (id,owner_user_id,contact_id,client_id) values ($1,$2,$3,$4)`, [ids.lead, ids.owner, ids.contact, ids.client]);
...
describe('Owner client account deletion RPC', { concurrency: false }, () => {
  ...
  test('deletes only the scoped portal account, detaches contacts/leads, cascades auth rows, and preserves audit rows', async () => {
    ...
    const { rows: [state] } = await db.query(`select (select count(*) from public.crm_clients where id=$1)::int clients,(select client_id is null from public.contacts where id=$2) contact_detached,(select client_id is null from public.crm_leads where id=$3) lead_detached,(select count(*) from public.client_portal_memberships where crm_client_id=$1)::int memberships,(select count(*) from public.client_portal_invitations where membership_id=$4)::int invitations,(select count(*) from public.users where id=$5)::int users,(select count(*) from public.sessions where user_id=$5)::int sessions,(select user_id is null from public.audit_logs where event_type='test' and action='before_delete') audit_preserved`, [ids.client, ids.contact, ids.lead, ids.membership, ids.portalUser]);
    assert.deepEqual(state, { clients: 0, contact_detached: true, lead_detached: true, memberships: 0, invitations: 0, users: 0, sessions: 0, audit_preserved: true });
