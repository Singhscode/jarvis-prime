...
let ownerEntitled = true;
globalThis.fetch = async (input, init = {}) => {
  const url = input instanceof Request ? input.url : String(input);
  const requestUrl = new URL(url);
  const isEntitlementQuery = requestUrl.pathname.endsWith('/owner_workspace_entitlements')
    && requestUrl.searchParams.get('select') === 'user_id'
    && requestUrl.searchParams.get('user_id') === `eq.${ownerId}`;
  if (isEntitlementQuery) {
    authorizationCalls.push(url);
    return json(ownerEntitled ? { user_id: ownerId } : null);
  }
...
describe('Owner Workspace', () => {
  test('requires the existing JWT and explicit Owner entitlement before bootstrap', async () => {
    workspaceIdentity = ownerIdentity(); ownerEntitled = true; authorizationCalls.length = 0;
    await withServer(async (port) => {
      ...
      workspaceIdentity = ownerIdentity(); ownerEntitled = false;
      ...
      response = await nativeFetch(`http://127.0.0.1:${port}/owner-workspace/projects`, {
        method: 'POST', headers: { Authorization: `Bearer ${token('client')}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ client_id: '30000000-0000-4000-8000-000000000003', name: 'Denied' }),
      });
      assert.equal(response.status, 403); assert.equal((await response.json()).error.code, 'INSUFFICIENT_PERMISSIONS');
    });
    assert.ok(authorizationCalls.some((url) => url.includes('/owner_workspace_entitlements?')));
    assert.ok(authorizationCalls.some((url) => url.includes('/users?')));
    ownerEntitled = true; workspaceIdentity = ownerIdentity();
  });
...
test('requires explicit Owner entitlement and stores only a hash when issuing employee invitations', async () => {
  calls.length = 0;
  let rpcBody;
  databaseFetch = (rawUrl, init = {}) => {
    const url = new URL(rawUrl);
    if (url.pathname.endsWith('/rpc/issue_employee_invitation')) {
      rpcBody = JSON.parse(init.body);
      return json({ id: '30000000-0000-4000-8000-000000000003', status: 'pending', expires_at: '2026-07-29T00:00:00.000Z' });
    }
    throw new Error(`Unexpected query: ${rawUrl}`);
  };
  ...
  await withServer(async (port) => {
    const headers = { Authorization: `Bearer ${token()}`, 'Content-Type': 'application/json' };
    ownerEntitled = false;
    let response = await nativeFetch(`http://127.0.0.1:${port}/owner-workspace/employees/invitations`, {
      method: 'POST', headers, body: JSON.stringify({ full_name: 'New Employee', email: 'employee@example.test' }),
    });
    assert.equal(response.status, 403);

    ownerEntitled = true;
    response = await nativeFetch(`http://127.0.0.1:${port}/owner-workspace/employees/invitations`, {
      method: 'POST', headers, body: JSON.stringify({ full_name: 'New Employee', email: 'Employee@Example.Test' }),
    });
    assert.equal(response.status, 201);
    const body = await response.json();
    assert.deepEqual(body.data.invitation, {
      id: '30000000-0000-4000-8000-000000000003',
      status: 'pending',
      expiresAt: '2026-07-29T00:00:00.000Z',
    });
    assert.doesNotMatch(JSON.stringify(body), /token|password|hash|owner_user_id/i);
  });

  assert.equal(rpcBody.p_owner_user_id, ownerId);
  assert.equal(rpcBody.p_email_normalized, 'employee@example.test');
  assert.equal(rpcBody.p_full_name, 'New Employee');
  assert.match(rpcBody.p_token_hash, /^[0-9a-f]{64}$/);
  assert.equal(Object.hasOwn(rpcBody, 'password'), false);
  ownerEntitled = true;
});