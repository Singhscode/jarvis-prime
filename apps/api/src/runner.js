...
import { listActiveClients, getProspectsByStage } from './database/db.js';
...
async function main() {
  try {
    ...
    if (task === 'source') { banner(); return await taskSource(); }
    if (task === 'outbound') { banner(); return await taskOutbound(); }
    if (task === 'inbound') { banner(); return await taskInboundDemo(); }
