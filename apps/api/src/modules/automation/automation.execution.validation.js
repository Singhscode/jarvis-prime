...
export const ACTION_CODES = Object.freeze(['ACT_ASSIGN', 'ACT_TASK', 'ACT_NOTIFY']);
...
export function classifyError(error, { afterDispatch = false, knownOutcome = false } = {}) { const code = String(error?.code || error?.message || '').toUpperCase(); if (afterDispatch && !knownOutcome) return { state: 'HUMAN_REVIEW', reason: 'POST_DISPATCH_UNCERTAIN' }; if (/TIMEOUT|UNAVAILABLE|ECONN|NETWORK|RATE_LIMIT|TEMPORARY/.test(code)) return { state: 'RETRYABLE', reason: 'TRANSIENT_DEPENDENCY_FAILURE' }; if (/DENIED|NOT_FOUND|VALIDATION|CONFLICT|PERMISSION|AUTH/.test(code)) return { state: 'FAILED', reason: 'TERMINAL_DOMAIN_FAILURE' }; return { state: 'HUMAN_REVIEW', reason: 'UNKNOWN_OUTCOME' }; }
