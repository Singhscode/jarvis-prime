...
export async function setOwnerControl(userId, values, rawIdempotency) {
  ...
  try {
    const outcome = await repository.setControl({ ownerUserId: actor.ownerUserId, scopeType: body.scopeType, scopeId, paused: body.paused, emergencyStop: body.emergencyStop, reasonCode: reason(body.reasonCode, 'OWNER_CONTROL'), actorUserId: actor.actorUserId, idempotencyKey: idempotency(rawIdempotency) });
    return { scopeType: body.scopeType, scopeId, paused: body.paused, emergencyStop: body.emergencyStop, replayed: Boolean(outcome?.replayed), eventId: outcome?.event_id };
  } catch (error) { mapError(error); }
}
