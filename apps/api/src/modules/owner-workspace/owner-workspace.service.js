...
export async function inviteEmployee(ownerUserId, values) {
  try {
    if (!values || typeof values !== 'object' || Array.isArray(values)) {
      throw new AppError('Employee name and email are required.', 400, 'VALIDATION_ERROR');
    }
    const keys = Object.keys(values);
    if (keys.some((key) => !['full_name', 'email'].includes(key))) {
      throw new AppError('Employee invitation fields are invalid.', 400, 'VALIDATION_ERROR');
    }
    ...
    if (!fullName || fullName.length > 120 || /[\u0000-\u001f\u007f]/.test(fullName)) {
      throw new AppError('Employee name is invalid.', 400, 'VALIDATION_ERROR');
    }
    const { generateToken, hashToken, isValidEmailFormat, normalizeEmail } = await import('../auth/crypto.js');
    if (email.length > 320 || !isValidEmailFormat(email)) {
      throw new AppError('Employee email is invalid.', 400, 'VALIDATION_ERROR');
    }

    const invitation = generateToken();
    const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
    const result = await repository.issueEmployeeInvitation(
      ownerUserId, normalizeEmail(email), fullName, hashToken(invitation), expiresAt
    );
    ...
    let activationUrl;
    try {
      activationUrl = new URL('/employee/activate', origin);
      activationUrl.searchParams.set('invitation', invitation);
    } catch {
      await repository.revokeEmployeeInvitation(ownerUserId, result.id).catch(() => {});
      throw new AppError('Employee invitation delivery failed.', 500, 'INTERNAL_ERROR', false);
    }
    const delivery = await sendTransactionalEmail({
      to: email,
      subject: 'Your JARVIS PRIME Employee Portal invitation',
      body: `Use this secure link to activate your Employee Portal access:\n${activationUrl.toString()}\n\nThis link expires in 24 hours.`,
    });
    if (!['sent', 'dry_run'].includes(delivery.status)) {
      await repository.revokeEmployeeInvitation(ownerUserId, result.id).catch(() => {});
      throw new AppError('Employee invitation delivery failed.', 500, 'INTERNAL_ERROR', false);
    }
    return { invitation: { id: result.id, status: result.status, expiresAt: result.expires_at } };
  } catch (error) {
    if (error instanceof AppError) throw error;
    if (error?.code === 'P0001' || error?.message === 'EMPLOYEE_INVITATION_UNAVAILABLE') {
      throw new AppError('Employee invitation could not be issued.', 409, 'EMPLOYEE_INVITATION_UNAVAILABLE');
    }
    throw new AppError('Employee invitation could not be issued.', 500, 'INTERNAL_ERROR', false);
  }
}
...
const AUDIT_CATEGORIES = { all: [], security: ['user.login', 'login.failed', 'account.locked'], invitations: ['client_portal_invitation', 'employee.invitation_issued', 'employee.invitation_activated', 'employee.invitation_delivery_failed'], documents: ['client_portal_document'] };
...
function auditLabel(event) {
  ...
  if (event.event_type === 'employee.invitation_issued') return 'Employee invitation issued';
  if (event.event_type === 'employee.invitation_activated') return event.success ? 'Employee invitation activated' : 'Employee invitation activation failed';
  if (event.event_type === 'employee.invitation_delivery_failed') return 'Employee invitation delivery failed';
