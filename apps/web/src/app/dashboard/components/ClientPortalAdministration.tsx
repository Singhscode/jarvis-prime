...
import type { ApiBody, ClientPortalAdministration as Portal, OwnerClient, OwnerContact, OwnerPage, OwnerRenameInput } from '../lib/owner-contracts';
...
export default function ClientPortalAdministration({ clientId }: Props) {
  ...
  const [clientName, setClientName] = useState(''); const [contactId, setContactId] = useState(''); const [membershipCursor, setMembershipCursor] = useState<string | null>(null); const [loading, setLoading] = useState(true); const [working, setWorking] = useState(false); const [error, setError] = useState(''); const [notice, setNotice] = useState('');
  const load = useCallback(async () => {
    ...
    try {
      ...
      setDetail(client.data); setClientName(client.data.client.name); setPortal(portalData.data);
    }
  ...
  async function updateClientName(event: React.FormEvent) {
    event.preventDefault(); if (working) return; setWorking(true); setError(''); setNotice('');
    try { const payload: OwnerRenameInput = { name: clientName.trim() }; await request(`/api/owner-workspace/clients/${clientId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); setNotice('Client name updated.'); await load(); }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Client update failed.'); }
  ...
  return <section><Link href="/dashboard/clients" className="text-sm text-cyan-300 hover:underline">← Clients</Link><header className="mt-4"><p className="text-sm font-medium text-cyan-300">Client details</p><h1 className="mt-1 text-3xl font-semibold">{detail?.client.name}</h1><p className="mt-2 text-sm text-slate-300">Created {format(detail?.client.created_at || null)}</p></header>
    ...
    <form onSubmit={(event) => void updateClientName(event)} className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/70 p-5"><h2 className="text-lg font-semibold">Edit client</h2><div className="mt-3 flex flex-wrap gap-3"><label className="text-sm">Client name<input required aria-label="Edit client name" value={clientName} onChange={(event) => setClientName(event.target.value)} className="ml-2 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-cyan-300" /></label><button disabled={working || !clientName.trim() || clientName.trim() === detail?.client.name} className="rounded-lg bg-cyan-300 px-4 py-2 text-sm font-semibold text-slate-950 focus:outline-none focus:ring-2 focus:ring-cyan-100 disabled:cursor-not-allowed disabled:opacity-60">{working ? 'Saving…' : 'Save client name'}</button></div></form>
