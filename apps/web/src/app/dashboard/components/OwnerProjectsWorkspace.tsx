...
export default function OwnerProjectsWorkspace() {
  ...
  const [sort, setSort] = useState('name:asc'); const [cursor, setCursor] = useState<string | null>(null); const [page, setPage] = useState<OwnerPage<OwnerProject> | null>(null); const [loading, setLoading] = useState(true); const [error, setError] = useState('');
  ...
  return <section><header className="mb-7"><p className="text-sm font-medium text-cyan-300">Projects</p><h1 className="mt-1 text-3xl font-semibold">Project overview</h1><p className="mt-2 text-sm text-slate-300">Projects are scoped to your authenticated workspace and their assigned client.</p></header>
