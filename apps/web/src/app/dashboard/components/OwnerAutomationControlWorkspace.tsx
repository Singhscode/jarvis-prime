...
export default function OwnerAutomationControlWorkspace() {
  ...
  return <section>
    ...
    {tab === 'overview' && (loading ? skeleton : <div className="space-y-6">
      ...
      <section className={CARD}><h2 className="text-lg font-semibold">Durable execution health</h2>{health?.operationalHealth ? <div className="mt-4 space-y-4 text-sm"><p className="text-slate-300">Control plane: <strong>{health.operationalHealth.compatibility.ready ? 'compatible' : 'unavailable'}</strong> · observed {formatTimestamp(health.operationalHealth.observedAt)}</p><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><div><p className="text-xs uppercase tracking-wide text-slate-400">Eligible work</p><p className="mt-1 text-xl font-semibold">{health.operationalHealth.queue.eligibleCount}</p></div><div><p className="text-xs uppercase tracking-wide text-slate-400">Active leases</p><p className="mt-1 text-xl font-semibold">{health.operationalHealth.leases.activeCount}</p></div><div><p className="text-xs uppercase tracking-wide text-slate-400">Stale leases</p><p className="mt-1 text-xl font-semibold">{health.operationalHealth.leases.staleCount}</p></div><div><p className="text-xs uppercase tracking-wide text-slate-400">Recovered, 24h</p><p className="mt-1 text-xl font-semibold">{health.operationalHealth.recovery.recoveredLast24h}</p></div></div><p className="text-xs text-slate-400">Oldest queued: {formatTimestamp(health.operationalHealth.queue.oldestQueuedAt)} · oldest eligible: {formatTimestamp(health.operationalHealth.queue.oldestEligibleAt)}. Worker process liveness is verified by its separately deployed supervisor probe and is not inferred from this browser view.</p></div> : <p className="mt-3 text-sm text-slate-300">Durable operational health is unavailable.</p>}</section>
