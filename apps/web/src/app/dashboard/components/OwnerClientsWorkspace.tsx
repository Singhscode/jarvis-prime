  ...
  async function inviteClient(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNotice(body.data.delivery.status === 'failed'
        ? 'Client invitation is ready, but delivery failed. Please try again later.'
        : 'Client invitation sent. The client can activate their account from the email.');
      resetClientAccountForm();
      setDialogOpen(false);
      await load({ query: '', sort: nextSort, cursor: null });
    } catch (caught) {
      setFormError(caught instanceof Error ? caught.message : 'Unable to invite the client.');
    } finally {
      setCreating(false);
    }
  }
