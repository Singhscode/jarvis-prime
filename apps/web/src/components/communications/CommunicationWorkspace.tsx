...
export default function CommunicationWorkspace({ request, role, heading, description }: Props) {
  ...
  const [creating, setCreating] = useState(false); const [subject, setSubject] = useState(''); const [employeeUserId, setEmployeeUserId] = useState(''); const [clientMembershipId, setClientMembershipId] = useState('');
  ...
  async function createThread(event: React.FormEvent) {
    ...
    const participants = [
      ...(employeeUserId.trim() ? [{ kind: 'employee', userId: employeeUserId.trim() }] : []),
      ...(clientMembershipId.trim() ? [{ kind: 'client', membershipId: clientMembershipId.trim() }] : []),
    ];
    ...
    try {
      ...
      setCreating(false); setSubject(''); setEmployeeUserId(''); setClientMembershipId(''); setBody('');
